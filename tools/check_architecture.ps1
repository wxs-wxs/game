...
$rules = @(
    ...
    [pscustomobject]@{ Pattern = '(?<![A-Za-z0-9_])(?!(?:self|inventory)(?:\.|\[|$))(?:\$?[A-Za-z_]\w*(?:\.[A-Za-z_]\w*|\[[^\]]+\])*)\.(?:amounts|backpack|storage|phase)(?:\[[^\]]+\])?\s*(?:\+=|-=|\*=|/=|%=|=(?!=))'; Rule = 'core/domain must not write another module field directly' }
    [pscustomobject]@{ Pattern = '(?<![A-Za-z0-9_])(?!(?:self|inventory)(?:\.|\[|$))(?:\$?[A-Za-z_]\w*(?:\.[A-Za-z_]\w*|\[[^\]]+\])*)\[\s*["'']?(?:amounts|backpack|storage|phase)["'']?\s*\](?:\[[^\]]+\])?\s*(?:\+=|-=|\*=|/=|%=|=(?!=))'; Rule = 'core/domain must not write another module field directly' }
)
