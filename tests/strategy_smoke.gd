...
func _init() -> void:
	...
	var policy_result := game.set_camp_policy("rationing")
	assert(bool(policy_result.get("ok", false)))
	assert(game.get_survival_status().get("policy", {}).get("id", "") == "rationing")
	var saved := game.to_dict()
	var restored := GameManager.new()
	restored.from_dict(saved)
	assert(restored.get_survival_status().get("policy", {}).get("id", "") == "rationing")
