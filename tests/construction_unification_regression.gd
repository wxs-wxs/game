...
func _init() -> void:
	...
	for id in ids:
		...
		assert(definition.has("icon"), id + " has a building icon key")
	...
	var busy_status := game.construction_status("bed")
	assert(busy_status.get("state", "") == "busy")
	assert(str(busy_status.get("reason", "")).contains("篝火"))
	var busy_preflight := game.buildings.construction_preflight("bed", game.resources, game.construction_skill.level)
	assert(not busy_preflight.get("ok", true))
	assert(str(busy_preflight.get("reason", "")).contains("篝火"))
