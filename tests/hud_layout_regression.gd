...
func _init() -> void:
	...
	for control in [ui.day_label, ui.clock_icon_label, ui.clock_label, ui.weather_icon_label, ui.weather_label, ui.threat_label]:
	...
	assert(ui.hud.get_node("ToolBar") != null)
	...
	assert(ui.tool_bar.position == Vector2(403, 480))
	assert(ui.tool_bar.size == Vector2(154, 48))
	assert(ui.axe_action_button.text == "")
	assert(ui.pickaxe_action_button.text == "")
	...
	for index in range(7):
		var rail := ui.hud.get_node("ActionRailButton%d" % index) as Button
		assert(rail != null)
		assert(rail.position.x == 916.0)
		assert(rail.position.y >= 57.0 and rail.position.y + rail.size.y <= 540.0)
		assert(rail.size == Vector2(32, 30))
