...
func _init() -> void:
	...
	var source_legacy := ResourceManager.new()
	source_legacy.amounts["food"] = 0
	source_legacy.storage["food"] = 0
	assert(source_legacy.collect_from_source("food", 1, "camp_task") == 1)
	assert(source_legacy.backpack["food"] == 1)
	assert(source_legacy.storage["food"] == 0)
