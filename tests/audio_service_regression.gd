...
func _init() -> void:
	...
	assert(service.play_sfx("resource_shortage") == "headless")
	assert(service.play_music("day") == "exploration_day")
	service.play_ambience("indoor_rain")
	assert(service.world_state.location == "interior")
	service.set_pause_ducked(true)
	assert(service.last_snapshot_targets["Music"] < base_volume_db(service, "Music"))
	service.set_pause_ducked(false)
