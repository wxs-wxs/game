...
HUD 采用 960×540 原生网格的边缘信息布局：左上显示天数、时间和天气，顶部中央显示威胁状态，右上以 Ninja Adventure 16px 图标横向展示资源；左下显示主角生存状态，右下显示当前目标与最近日志。操作采用快捷键，画面不再常驻右侧操作栏。所有标准面板和按钮使用 Kenney Pixel UI 九宫格，文本使用 Fusion Pixel，图标保持最近邻和整数尺寸。按 `K` 打开背包，按 `B` 打开建造选择面板，按左上角 `?` 查看快捷键浮层，交互点有唯一 ID、范围、耗时、资源消耗、奖励、失败概率和冷却时间。
...
每个探索日会生成一个每日目标。完成目标会获得资源奖励并积累连胜，未完成会提高营地威胁；威胁过高时可能触发夜袭。游戏不会在第 7 天自动结束，除主动退出外只有主角阿禾的生命值降为 0 时才会结束。体温、里程碑、政策效果和策略状态会随存档保存。
...
每天从清晨出发，白天可以在荒野采集、钓鱼、搜废墟或建造。时间走到傍晚后不会自动传送，玩家要自己回到小屋床边按 `E`，才会进行一次夜间结算。结算会处理食物、体温、威胁、篝火燃料和当晚事件，然后显示报告；按“进入清晨”才开始下一天。

游戏没有第七天节点，也没有固定天数胜利。只要主角还活着，就可以一直继续；主角生命值归零才会结束。天气目前只影响温度、威胁和事件权重，不改变当天采集数量。水资源保留为兼容数据，本阶段不加入补水消耗闭环。
